...
import { getTapiceriaCategoryGallery } from '@/assets/images'
...
export default function Tapiceria() {
  const { content, heroImage, portfolioProjects, tabs, seo, source, showcaseImages, isLoading } =
    useServicePageDisplay('tapiceria')
  ...
  return (
    <>
      <PageMeta page="tapiceria" cmsSeo={seo ?? undefined} />
        ...
        <Section className={showIntro ? 'bg-white' : undefined}>
          ...
          >
            <BanquetasCategoryPanel
              tabs={tabs}
              cmsFirst={isCms}
              resolveCategoryGallery={getTapiceriaCategoryGallery}
              ariaLabel="Servicios de tapicería"
              tabIndicatorId="tapiceria-tab-indicator"
            />
          </motion.div>
        </Section>
      ) : null}
