...
const PAGE_KEY = 'proteccion-cabina'

function SpecBlock({ title: blockTitle, items }) {
  return (
    <div className="rounded-lg border border-border bg-white p-5 sm:p-6">
      <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-ink">{blockTitle}</h3>
...
export default function ProteccionCabina() {
  ...
  return (
    <>
      <PageMeta page={PAGE_KEY} cmsSeo={seo ?? undefined} />
      <ServicePageHero eyebrow={hero.eyebrow} title={hero.title} subtitle={hero.subtitle} highlights={hero.highlights} image={heroImage} imageAlt={hero.imageAlt} />

      <ServicePageShowcaseSection showcase={content.showcase} images={showcaseImages} />

      <Section>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease }} className="mx-auto max-w-3xl">
          <SectionHeader eyebrow={intro.eyebrow} title={intro.title} className="mb-8" />
          <div className="space-y-5">
            {(intro.paragraphs ?? []).map((paragraph) => (
              <p key={paragraph} className="text-base leading-relaxed text-ink-muted sm:text-lg">{paragraph}</p>
            ))}
          </div>
        </motion.div>
      </Section>
      <Section className="bg-white">
        <SectionHeader eyebrow={specs.eyebrow} title={specs.title} description={specs.description} align="center" className="mx-auto max-w-2xl" />
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease }} className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6">
          {(specs.sections ?? []).map((block) => (
            <SpecBlock key={block.title} title={block.title} items={block.items ?? []} />
          ))}
        </motion.div>
      </Section>
      <ServicePagePortfolio pageKey={PAGE_KEY} eyebrow={gallery.eyebrow} title={gallery.title} description={gallery.description} projects={portfolioProjects ?? []} />
